...
word2vec_model = KeyedVectors.load_word2vec_format('/Users/haresh/Downloads/Connections AI 2/gensim-data/word2vec-google-news-300/word2vec-google-news-300', binary=True)
...
last_guess = None
one_away_words = set()  # Track words that were part of one-away guesses
...
def model(words, strikes, isOneAway, correctGroups, previousGuesses, error):
    ...
    for combination in combinations(range(len(available_words)), 4):
        ...
        if one_away_overlap >= 2:  # If at least 2 words were part of a one-away guess
    ...
    # If we have any valid groups with good similarity, return the best one
    if potential_groups and potential_groups[0][1] > 0.5:  # Only use groups with decent similarity
